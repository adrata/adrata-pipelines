...
function mapToAdvancedCSV(result) {
    return {
        "Website": result.website || 'Not available',
        "Company Name": result.companyName || 'Not available',
        "Industry": result.industry || 'Technology Services',
        "Industry Vertical": result.industryVertical || 'Business Technology',
        "Executive Stability Risk": 'Requires executive research',
        "Deal Complexity Assessment": 'Requires company size research',
        "Competitive Context Analysis": 'Requires competitive landscape research',
        "Industry Analysis Reasoning": 'Analysis based on company domain and name patterns',
        "Account Owner": result.accountOwner || 'Not available'
    };
}
...
function mapToPowerhouseCSV(result) {
    return {
        "Website": result.website || 'Not available',
        "Company Name": result.companyName || 'Not available',
        "Decision Maker": result.cfo?.name || 'Requires CFO research',
        "Decision Maker Role": 'Chief Financial Officer',
        "Champion": 'Requires CEO research',
        "Champion Role": 'Chief Executive Officer',
        "Stakeholder": 'Requires CTO research',
        "Stakeholder Role": 'Chief Technology Officer',
        "Blocker": 'Requires organizational analysis',
        "Blocker Role": 'Potential Blocker - TBD',
        "Introducer": result.cro?.name || 'Requires CRO research',
        "Introducer Role": 'Chief Revenue Officer',
        "Budget Authority Mapping": 'Requires financial authority research',
        "Procurement Maturity Score": 'Requires procurement analysis',
        "Decision Style Analysis": 'Requires organizational decision-making analysis',
        "Sales Cycle Prediction": 'Requires deal complexity analysis',
        "Buyer Group Flight Risk": 'Requires executive stability analysis',
        "Routing Intelligence Strategy 1": 'Requires executive engagement strategy',
        "Routing Intelligence Strategy 2": 'Requires stakeholder relationship mapping',
        "Routing Intelligence Strategy 3": 'Requires organizational influence analysis',
        "Routing Intelligence Explanation": `${result.companyName} buyer group analysis requires comprehensive research`,
        "Buyer Group Analysis Reasoning": 'Buyer group analysis requires executive research and organizational mapping',
        "Account Owner": result.accountOwner || 'Not available'
    };
}
